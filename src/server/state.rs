...
impl TaskProcessor<EmbeddingsRequest, EmbeddingsResponse> for EmbeddingsProcessor {
	...
	fn handle_task(&mut self, request: EmbeddingsRequest) -> Result<EmbeddingsResponse> {
        ...
        // Infer embeddings
        let (embeddings, usage) = self.sentence_transformer.encode_batch_with_usage(sentences, normalize)?;

        let response = EmbeddingsResponse::from_embeddings(
            embeddings, usage, request.model
        );
...
impl ServerState {
	pub fn new(queue: &Queue<EmbeddingsRequest, EmbeddingsResponse, EmbeddingsProcessor>) -> Result<Self> {
